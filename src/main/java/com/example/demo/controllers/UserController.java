...
public class UserController {
	...
	public ResponseEntity<User> createUser(@RequestBody CreateUserRequest createUserRequest) {
		...
		if(createUserRequest.getUsername() == null || createUserRequest.getPassword() == null || createUserRequest.getConfirmPassword() == null){
			log.error("Attempt failed. The username or password field is " +
					"empty!");
			return ResponseEntity.badRequest().build();
		}

		if(!createUserRequest.getPassword().equals(createUserRequest.getConfirmPassword())){
			log.error("Attempt failed. Password and Confirm Password do not " +
					"match!");
			return ResponseEntity.badRequest().build();
		}

		if(createUserRequest.getConfirmPassword().length() < 7){
			log.error("Attempt failed. Cannot create user, password must be " +
					"over 7 characters" +
					". You provided {}.",
